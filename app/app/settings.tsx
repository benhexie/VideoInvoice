...
const TemplatePreview = ({ type, color }: { type: string; color: string }) => {
  ...
  if (type === "premium") {
    return (
      <View style={[tpStyles.container, { backgroundColor: "#fafafa" }]}>
  ...
  if (type === "modern") {
    return (
      <View style={[tpStyles.container, { backgroundColor: "#fff", borderRadius: 6, overflow: "hidden" }]}>
...
export default function SettingsScreen() {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState(initialState.template);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const finalPriceListUrl = priceListUrl ? await uploadFileToStorage(priceListUrl, "price_lists", priceListName || "pricelist.pdf") : "";
  ...
  const openTemplatePreview = async (templateId: string) => {
    ...
    try {
      ...
      html = html.replace(/<meta\s+name=["']viewport["']\s+content=["'][^"']*["']\s*\/?>/i, '<meta name="viewport" content="width=1024">');
      setPreviewHtml(html);
    } catch (e) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        ...
        >
          <Animated.View entering={FadeInDown.duration(600)}>
            ...
            <Text style={styles.headerSubtitle}>Update your default invoice template and business details.</Text>
          </Animated.View>
              ...
              {TEMPLATES.map((tmpl) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={[styles.templateName, isSelected && { color: tmpl.color, fontWeight: "bold" }]}>{tmpl.name}</Text>
                    {isSelected && (
          ...
          <Animated.View entering={FadeInDown.delay(300).duration(600)} style={styles.section}>
            ...
            <Text style={styles.sectionSubtitle}>Choose a primary color for your invoice elements.</Text>
          ...
          <Animated.View entering={FadeInDown.delay(400).duration(600)} style={styles.section}>
            ...
            >
              ...
              <View style={[styles.input, { justifyContent: "center" }]}>
                <Text style={{ color: currency ? colors.textPrimary : colors.textSecondary, fontSize: 16 }}>
              ...
              <Animated.View entering={FadeInDown.duration(400)} style={{ marginTop: 16 }}>
                <View style={styles.signatureCanvas}>
                  ...
                  <View style={styles.signatureCanvasFooter}>
                    <TouchableOpacity onPress={() => { setShowSignatureCanvas(false); setScrollEnabled(true); }} style={styles.sigBtnCancel}>
            ...
            {!showSignatureCanvas && (
              <>
                <View style={{ marginTop: 24, marginBottom: 12 }}>
                  <Text style={styles.uploadLabel}>Price List</Text>
                  <Text style={styles.uploadSubLabel}>Upload your rates so AI uses your exact prices. Supports PDF, CSV, Excel, Word, and more.</Text>
                </View>
                {priceListUrl ? (
                  <View style={[styles.logoUploadBtn, { flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 16 }]}>
                    <View style={{ flexDirection: "row", alignItems: "center", flex: 1, gap: 10 }}>
                      ...
                      <Text style={[styles.logoUploadText, { flex: 1 }]} numberOfLines={1} ellipsizeMode="middle">
                        {priceListName || "Price list uploaded"}
                      </Text>
                    </View>
                    <TouchableOpacity onPress={() => { setPriceListUrl(""); setPriceListName(""); }} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
                      <X color={colors.textSecondary} size={18} />
                    </TouchableOpacity>
                  </View>
                ) : (
                  <TouchableOpacity style={styles.logoUploadBtn} onPress={handlePriceListUpload}>
                    <FileText color={colors.accent} size={24} style={{ marginRight: 8 }} />
        ...
        <View style={styles.footer}>
          <TouchableOpacity
            style={[styles.saveBtn, (loading || !hasChanges) && { opacity: 0.7, backgroundColor: !hasChanges ? colors.surfaceRaised : colors.accent }]}
            onPress={handleSave}
          ...
          >
            {loading ? (
              ...
              <Text style={[styles.saveBtnText, !hasChanges && { color: colors.textSecondary }]}>Save Changes</Text>
            )}
          </TouchableOpacity>
              ...
              <ScrollView style={styles.currencyList}>
                {currencySearchResults.length > 0 ? (
                  ...
                  <Text style={styles.emptySearchText}>
                    {currencySearchQuery.trim() === "" ? "Type to search currencies..." : "No currencies found"}
                  </Text>
                )}
              </ScrollView>
            </RNAnimated.View>
      ...
      <Modal
        ...
        transparent={false}
        animationType="slide"
        onRequestClose={() => { setPreviewTemplateId(null); setPreviewHtml(null); }}
      >
        <View style={{ flex: 1, backgroundColor: colors.background }}>
          <View style={[styles.templatePreviewModalHeader, { paddingTop: insets.top + 14 }]}>
            <TouchableOpacity
              onPress={() => { setPreviewTemplateId(null); setPreviewHtml(null); }}
          ...
          <View style={{ flex: 1, backgroundColor: "#fff" }}>
            {previewLoading ? (
              <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
                <ActivityIndicator size="large" color={colors.accent} />
                <Text style={{ marginTop: 12, color: colors.textSecondary, fontSize: 15 }}>Loading preview...</Text>
              </View>
            ) : previewHtml ? (
              <WebView originWhitelist={["*"]} source={{ html: previewHtml }} style={{ flex: 1 }} scalesPageToFit={true} />
            ) : null}
          </View>
        </View>
      </Modal>
...
const createStyles = (c: AppColors) => StyleSheet.create({
  ...
  sectionSubtitle: { fontSize: 14, color: c.textSecondary, marginBottom: 16, marginTop: -8 },

  colorPalette: { flexDirection: "row", gap: 12, paddingBottom: 8, paddingTop: 8, paddingHorizontal: 8 },
  colorSwatch: { width: 44, height: 44, borderRadius: 22, justifyContent: "center", alignItems: "center", borderWidth: 2, borderColor: "transparent" },
  colorSwatchSelected: { borderColor: c.textPrimary, transform: [{ scale: 1.1 }] },
  ...
  signatureCanvas: {
    height: 260, backgroundColor: c.surface, borderRadius: 16, overflow: "hidden", borderWidth: 1, borderColor: c.border,
  },
  signatureCanvasHeader: { padding: 12, borderBottomWidth: 1, borderBottomColor: c.border, backgroundColor: c.surface },
  ...
  sigBtnCancel: { paddingVertical: 8, paddingHorizontal: 16, borderRadius: 8, borderWidth: 1, borderColor: c.borderSubtle },
  sigBtnCancelText: { color: c.textSecondary, fontWeight: "600" },
  sigBtnClear: { paddingVertical: 8, paddingHorizontal: 16, borderRadius: 8, backgroundColor: c.surfaceRaised, borderWidth: 1, borderColor: c.borderSubtle },
  ...
  cancelButton: { backgroundColor: "transparent", paddingVertical: 16, borderRadius: 12, alignItems: "center", marginTop: 12 },
  cancelButtonText: { color: c.textSecondary, fontSize: 16, fontWeight: "600" },
  modalOverlay: { flex: 1, backgroundColor: c.overlay, justifyContent: "flex-end" },
  modalContent: {
    backgroundColor: c.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24,
    height: "80%", padding: 24, paddingTop: 12,
  },
  ...
  searchInputGroup: {
    flexDirection: "row", alignItems: "center", backgroundColor: c.surfaceRaised,
    borderRadius: 12, paddingHorizontal: 16, marginBottom: 16, height: 50,
  },
  ...
  templateEyeBtn: {
    ...
    borderRadius: 8, padding: 5, justifyContent: "center", alignItems: "center",
  },
  templatePreviewModalHeader: {
    flexDirection: "row", alignItems: "center", justifyContent: "space-between",
    paddingHorizontal: 16, paddingBottom: 14, backgroundColor: c.background,
    borderBottomWidth: 1, borderBottomColor: c.border,
  },
  templatePreviewModalClose: {
    width: 36, height: 36, borderRadius: 18, backgroundColor: c.surfaceRaised,
    justifyContent: "center", alignItems: "center",
  },
  templatePreviewModalTitle: { color: c.textPrimary, fontSize: 17, fontWeight: "700" },
  templatePreviewModalSelect: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 10 },
  templatePreviewModalSelectText: { color: "#fff", fontSize: 14, fontWeight: "700" },
});
